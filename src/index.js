...
function createCountry (item) {
   return `<H2><img class='flag'src="${item.flags.svg}" alt='Flag of this country'>
${item.name.official}</H2>
  <ul class='list'
    ...
    <li><span class='descr'>Languages:</span> ${Object.values(item.languages)}</li>
  </ul>`
}
...
const generateContent = (array, fn) => (array.reduce((acc, item) => acc + fn(item), ""));
